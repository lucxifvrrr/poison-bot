...
class Purge(commands.Cog):
    def __init__(self, bot):
        ...
        # Emojis
        self.INFO_EMOJI = "<:alert:1426440385269338164>"
        self.SUCCESS_EMOJI = "<a:white_tick:1426439810733572136>"
    ...
    async def delete_messages_with_retry(self, channel, messages: List[discord.Message], max_retries: int = 3) -> Tuple[bool, str]:
        ...
        if not messages:
            return False, f"{self.INFO_EMOJI} | No messages to delete."

        if len(messages) == 1:
            try:
                await messages[0].delete()
                return True, f"{self.SUCCESS_EMOJI} | Successfully deleted 1 message."
            except discord.HTTPException as e:
                return False, f"{self.INFO_EMOJI} | Failed to delete message: {str(e)}"

        current_delay = 1.0
        for attempt in range(max_retries):
            try:
                await channel.delete_messages(messages)
                return True, f"{self.SUCCESS_EMOJI} | Successfully deleted {len(messages)} messages."
        ...
        return False, f"{self.INFO_EMOJI} | Max retry attempts reached."
    ...
    async def purge_prefix(self, ctx, arg: str = None):
        if not arg:
            await ctx.send(f"{self.INFO_EMOJI} | Please specify the number of messages to delete or a filter type (bots/humans/@user).")
            return
        ...
        if arg.isdigit():
            ...
            try:
                ...
                await ctx.send(f"{self.INFO_EMOJI} | Invalid user mention. Please try again.", delete_after=3)
                return
        else:
            await ctx.send(f"{self.INFO_EMOJI} | Invalid argument. Use a number (1-100), 'bots', 'humans', or @user.", delete_after=3)
            return
        ...
        if not messages:
            await ctx.send(
                f"{self.INFO_EMOJI} | No messages found to delete. {too_old_count} messages were too old (>{self.MESSAGE_AGE_LIMIT} days).",
                delete_after=5
            )
            return
