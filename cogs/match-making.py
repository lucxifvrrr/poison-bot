...
class MatchPanel(discord.ui.View):
    ...
    async def start_match(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            print(f"Database error in queue for user {interaction.user.id} in guild {interaction.guild.id}:")
            print(f"Error: {str(e)}")
            ...
            print(f"Unexpected queue error:")
            ...
            embed.description = "An unexpected error occurred. This has been logged and will be investigated."
            await safe_reply(interaction, embed=embed, ephemeral=True)
...
class ReportModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            print(f"Error in report modal: {e}")
            try:
...
class Matchmaker(commands.Cog):
    ...
    async def enqueue(self, guild_id: int, user_id: int):
        ...
        try:
            async with db_pool.get() as db:
                ...
                if await cur.fetchone():
                    print(f"User {user_id} is already in queue for guild {guild_id}")
            ...
            try:
                async with db_pool.get() as db:
                    try:
                        ...
                        await db.commit()
                        print(f"Successfully enqueued user {user_id} in guild {guild_id}")
                        return
                    except Exception as e:
                ...
                last_error = e
                print(f"Database error (attempt {attempt + 1}/{max_retries}):")
                ...
                last_error = e
                print(f"Unexpected error (attempt {attempt + 1}/{max_retries}):")
                print(f"User ID: {user_id}")
                print(f"Guild ID: {guild_id}")
                print(f"Error: {str(e)}")
                print(f"Error type: {type(e).__name__}")
        ...
        print(f"Failed to enqueue user {user_id} after {max_retries} attempts. Last error: {str(last_error)}")
        raise last_error
                    ...
                    async with db_pool.get() as db:
                        ...
                        for row in rows:
                            ...
                            try:
                                ...
                                print(f"Error cleaning up thread {thread_id}: {str(e)}")
                                continue
                        
                        await db.commit()
                        
                except Exception as e:
                    print(f"Error cleaning up threads for guild {guild.id}: {str(e)}")
                    continue
                    
        except Exception as e:
            print(f"Error in cleanup_inactive_threads: {str(e)}")
